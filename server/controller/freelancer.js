...
Router.post("/create", auth, upload.single("fl_image"), async (req, res) => {
  ...
  // if (req.files.fl_image) {
  //   let path = req.files.fl_image[0].path.replaceAll("\\", "/");
  //   req.body.fl_image = path;
  // }
...
Router.put("/update/:id", async (req, res) => {
  ...
  try {
    ...
    res.json({ status: "success", message: "Freelance was updated" });
  } catch {
    res.json({ status: "danger", message: "Freelance was not renewed" });
  }
});
